 ...
 */
@AllArgsConstructor
public class AUpdater<T extends Wrapper<?>, L extends AContainer, U extends IUpdatableContainer<T, L>> implements IUpdater<T, L> {
    ...
    public Optional<L> getWrappedInstance(T wrappedInstance) {
        ...
        try {
            ...
            U updatableContainer = wrapInstanceToUpdatableContainerStrategy.wrapInstanceToUpdatableContainer(wrappedInstance);
    ...
    public Stream<? extends AContainer> update(L containerToUpdate, int currentFrame) {
        ...
        try {
            ...
            if (containerToUpdate instanceof IUpdatableContainer) {
                ...
                if (toUpdate != null && (!updates.containsKey(toUpdate) || updates.get(toUpdate) < currentFrame)) {
    ...
    public Optional<T> getBWInstance(L container) {
        try {
            ...
            if (container instanceof IUpdatableContainer) {
                return Optional.ofNullable(register.get(((IUpdatableContainer) container).getWrappedSCInstance())).map(IUpdatableContainer::getWrappedSCInstance);
            }
    ...
    public int getDeltaUpdate(L containerToUpdate, int currentFrame) {
        if (getBWInstance(containerToUpdate).isPresent()) {
            try {
                ...
                return lastUpdate.map(integer -> integer >= currentFrame ? 0 : currentFrame - integer).orElse(Integer.MAX_VALUE);
            } finally {
                lockForMaps.readLock().unlock();
            }
        }
