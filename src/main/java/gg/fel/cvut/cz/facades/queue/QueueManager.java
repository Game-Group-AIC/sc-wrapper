...
public class QueueManager implements ICommandExecutor {
  ...
  /**
   * Execute only commands to update all states
   */
  public QueueManager(ExecutionStrategy executionStrategy) {
   ...
   */
  public QueueManager() {
    this.executionStrategy = timeResources -> {
      ...
      Set<Queue> queuesToSkip = new HashSet<>(toExecuteNext);
        ...
        while (true) {
          ...
          for (int i = 0; i < queues; i++) {
            ...
            if (queue.canBeExpectedToBeExecuteInInterval(
              ...
              //higher priority is assigned
              if (!queuesToSkip.contains(queue)) {
                toExecute.add(queue);
                queuesToSkip.add(queue);
              }
            }
          }
  ...
  public void executedCommands(long timeResources) {
    executionStrategy.executedCommands(timeResources);
  }
  ...
  /**
   * Contract for execution strategy
   */
