...
public interface IPlayer extends InGameInterface, Serializable {
   ...
   *
   * Returns An integer representing the ID of the
   * player.
   */
   ...
   *
   * Returns A std::string object containing the player's name.
   ...
   * refineries are not included in this set.
   *
   * Returns Unitset containing all known units in the
   * game.
   */
   ...
   *
   * Returns Bulletset containing all accessible
   * IBullet objects.
   */
   ...
   * Races::Random when the game started and they have not been seen.
   *
   * Returns The IRace that the
   ...
   * human players.
   *
   * Returns The PlayerTypeEnum that identifies who is controlling a player. Note
   ...
   *
   * Returns true if this player has achieved victory,
   * otherwise false
   */
   ...
   *
   * Returns true if the player is defeated, otherwise
   * false
   */
   ...
   *
   * Returns true if the player has left the game, otherwise
   * false
   */
   ...
   * return 0 if the player is inaccessible.
   *
   * Returns Amount of minerals that the player currently
   * has for spending.
   */
   ...
   * return 0 if the player is inaccessible.
   *
   * Returns Amount of gas that the player currently has for
   * spending.
   */
   ...
   * This function will return 0 if the player is inaccessible.
   *
   * Returns Cumulative amount of gas
   ...
   * function will return 0 if the player is inaccessible.
   *
   * Returns Cumulative amount of minerals
   ...
   * function will return 0 if the player is inaccessible.
   *
   * Returns Cumulative amount of gas that the
   ...
   * current race will be used.
   *
   * Returns The supply that is in use for this player and the given
   ...
   *
   * Returns The total number of units of the given
   ...
   * then it will use UnitTypes::AllUnits by default.
   *
   * Returns The number of incomplete units of the
   ...
   * UnitTypes::AllUnits by default.
   *
   * Returns The total number of units that have died throughout the
   * game.
   */
   ...
   * UnitTypes::AllUnits by default.
   *
   * Returns The total number of units that the player has killed
   * throughout the game.
   */
   ...
   * Parameters upgrade The IUpgradeType to query.
   *
   * Returns The number of levels that the upgrade
   * has
   ...
   * to query.
   *
   * Returns true if the player has obtained the given tech, or false if they have not See
   ...
   * query.
   *
   * Returns true if the player is currently researching the tech, or false otherwise See
   ...
   * Parameters unit The IUnitType to retrieve the top speed for.
   *
   * Returns Top speed of the provided
   ...
   * consideration. Parameters weapon The IWeaponType to retrieve the maximum range for.
   *
   * Returns
   ...
   * consideration. Parameters unit The IUnitType to retrieve the sight range for.
   *
   * Returns Sight
   ...
   * consideration. Parameters unit The IUnitType to retrieve the damage cooldown for.
   *
   * Returns
   ...
   * unit type to calculate armor for, using the current player's upgrades.
   *
   ...
   * weapon type to calculate for.
   *
   * Returns The amount of damage that the weapon deals with this
   * player's upgrades.
   */
  ...
  /**
   * Retrieves the player's custom score. This score is used in Use Map Settings game types.
   *
   * Returns
   ...
   * observer player does not participate in the game.
   *
   * Returns true if the player is observing, or
   * false if the player is capable of playing in the game.
   */
  Optional<Boolean> isObserver();

  /**
   * Checks if a technology can be researched by the player. Certain technologies may be disabled in
   * Use Map Settings game types.
   *
   * Returns true if the tech type is available to the player for research.
   */
  default Optional<Boolean> isResearchAvailable(TechTypeEnum tech) {
    UnitTypeEnum unit = IGameTypes.UPGRADE_RESEARCHED_AT.get(tech);
    return getUnits().map(iUnitStream -> iUnitStream
        .anyMatch(iUnit -> iUnit.getType()
            .map(unitType -> unitType.getUnitType() == unit)
            .orElse(false)
        )
    );
  }
  ...
  /**
   * Verifies that this player satisfies a unit type requirement. This verifies complex type
   * requirements involving morphable Zerg structures. For example, if something requires a Spire,
   * but the player has (or is in the process of morphing) a Greater Spire, this function will
   * identify the requirement. It is simply a convenience function that performs all of the
   * requirement checks.
   *
   * Returns true if the unit type requirements are met, and false
   * otherwise. Since 4.1.2
   */
  // todo:
  Optional<Boolean> hasUnitTypeRequirement(UnitTypeEnum unit);
  // todo:
  Optional<Boolean> hasUnitTypeRequirement(UnitTypeEnum unit, int amount);

  /**
   * Checks all the requirements in order to make a given unit type for the current player. These
   * include resources, supply, technology tree, availability, and required units.
   *
   * Returns true indicating that the type can be made. If builder is provided, then it
   * is only true if builder can make the type. Otherwise it will return false, indicating that the
   * unit type can not be made.
   */
  // todo:
  Optional<Boolean> canMake(UnitTypeEnum type);

  /**
   * Checks all the requirements in order to research a given technology type for the current
   * player. These include resources, technology tree, availability, and required units. Parameters
   * type The ITechType to check. unit (optional) The IUnit that will be used to research the
   * provided technology type. If this value is nullptr or excluded, then the unit will be excluded
   * in the check. checkCanIssueCommandType (optional) TODO fill this in
   *
   * Returns true indicating
   * that the type can be researched. If unit is provided, then it is only true if unit can research
   * the type. Otherwise it will return false, indicating that the technology can not be
   * researched.
   */
  // todo:
  Optional<Boolean> canResearch(TechTypeEnum type);
