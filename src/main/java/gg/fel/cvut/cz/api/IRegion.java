 ...
 * accessibility to neighboring terrain, be used in general pathing algorithms, and used as nodes to
...
public interface IRegion extends IAbstractPoint, InGameInterface, Serializable {
  ...
  /**
   * Retrieves the center of the region. This position is used as the node of the region. Returns A
   * IPosition indicating the center location of the IRegion, in pixels.
   */
  Optional<IPosition> getPosition();

  /**
   * Retrieves a IUnit set containing all the units that are in this region. Also has the ability to
   * filter the units before the creation of the IUnit set. Parameters pred (optional) If this
   * parameter is used, it is a UnitFilter or function predicate that will retrieve only the units
   * whose attributes match the given criteria. If omitted, then a default value of nullptr is used,
