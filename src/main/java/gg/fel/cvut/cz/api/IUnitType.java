...
public interface IUnitType extends InGameInterface, Serializable {
    ...
    default Optional<Boolean> isRepairableMechanically() {
        return Optional.of((isBuilding().isPresent() && isBuilding().get()) || (isMechanical().isPresent() && isMechanical().get()));
    }

    default Optional<Boolean> isHealable() {
        return Optional.of((isOrganic().isPresent() && isOrganic().get()) || (isWorker().isPresent() && isWorker().get()));
    }
