...
public interface IUnitType {
     ...
     */
    Pair<IUnitType, Integer> whatBuilds();
     ...
     */
    Map<IUnitType, Integer> requiredUnits();

    /**
     * Identifies the required ITechType in order to create certain units. Note The only unit that requires a technology is the Lurker, which needs Lurker Aspect. Returns ITechType indicating the technology that must be researched in order to create this unit type. Return values TechTypes::None If creating this unit type does not require a technology to be researched.
     */
    ITechType requiredTech();

    /**
     * Retrieves the cloaking technology associated with certain units. Returns ITechType referring to the cloaking technology that this unit type uses as an ability. Return values TechTypes::None If this unit type does not have an active cloak ability.
     */
    ITechType cloakingTech();
     ...
     */
    List<IUpgradeType> upgrades();

    /**
     * Retrieves the upgrade type used to increase the armor of this unit type. For each upgrade, this unit type gains +1 additional armor. Returns IUpgradeType indicating the upgrade that increases this unit type's armor amount.
     */
    IUpgradeType armorUpgrade();
    ...
    /**
     * Retrieves this unit type's weapon type used when attacking targets on the ground. Returns IWeaponType used as this unit type's ground weapon. See also maxGroundHits, airWeapon
     */
    IWeaponType groundWeapon();
    ...
    /**
     * Retrieves this unit type's weapon type used when attacking targets in the air. Returns IWeaponType used as this unit type's air weapon. See also maxAirHits, groundWeapon
     */
    IWeaponType airWeapon();
    ...
    /**
     * Retrieves the set of technologies that this unit type is capable of researching. Note Some maps have special parameters that disable certain technologies. Use PlayerInterface::isResearchAvailable to determine if a technology is actually available in the current game for a specific player. Returns ITechType::set containing the technology types that can be researched. See also PlayerInterface::isResearchAvailable Since 4.1.2
     */
    List<ITechType> researchesWhat();

    /**
     * Retrieves the set of upgrades that this unit type is capable of upgrading. Note Some maps have special upgrade limitations. Use PlayerInterface::getMaxUpgradeLevel to check if an upgrade is available. Returns IUpgradeType::set containing the upgrade types that can be upgraded. See also PlayerInterface::getMaxUpgradeLevel Since 4.1.2
     */
    List<IUpgradeType> upgradesWhat();

    /**
     * Returns max shoot range (in build tiles) of this unit against land targets.
     */
    default double getShootRangeGround() {
        return groundWeapon().maxRange() / SIZE_IN_PIXELS;
    }

    /**
     * Returns max shoot range (in build tiles) of this unit against air targets.
     */
    default double getShootRangeAir() {
        return airWeapon().maxRange() / SIZE_IN_PIXELS;
    }

    /**
     * Returns max shoot range (in build tiles) of this unit against given <b>opponentUnit</b>.
     */
    default double getShootRangeAgainst(IUnitType type) {
        if (type.isFlyer()) {
     ...
     */
    default boolean canAttackGroundUnits() {
        return !groundWeapon().getWeaponType().equals(WeaponType.None);
    }

    /**
     * Returns true if unit has anti-air weapon.
     */
    default boolean canAttackAirUnits() {
        return !airWeapon().getWeaponType().equals(WeaponType.None);
    }
    ...
    default boolean isRepairableMechanically() {
        return isBuilding() || isMechanical();
    }

    default boolean isHealable() {
        return isOrganic() || isWorker();
    }

    default boolean canBeHealed() {
