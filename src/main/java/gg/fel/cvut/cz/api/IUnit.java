...
public interface IUnit {
    ...
    /**
     * Retrieves the player that owns this unit. Return values IGame::neutral() If the unit is a neutral unit or inaccessible. Returns The owning IPlayer interface object.
     */
    IPlayer getPlayer();

    /**
     * Retrieves the unit's type. Return values UnitTypes::Unknown if this unit is inaccessible or cannot be determined. Returns A IUnitType objects representing the unit's type. See also getInitialType
     */
    IUnitType getType();

    /**
     * Retrieves the unit's position from the upper left corner of the map in pixels. The position returned is roughly the center if the unit. Note The unit bounds are defined as this value plus/minus the values of IUnitType::dimensionLeft, IUnitType::dimensionUp, IUnitType::dimensionRight, and IUnitType::dimensionDown, which is conveniently expressed in UnitInterface::getLeft, UnitInterface::getTop, UnitInterface::getRight, and UnitInterface::getBottom respectively. Return values Positions::Unknown if this unit is inaccessible. Returns IPosition object representing the unit's current position. See also getTilePosition, getInitialPosition, getLeft, getTop
     */
    IPosition getPosition();

    /**
     * Retrieves the unit's build position from the upper left corner of the map in tiles. Note : This tile position is the tile that is at the top left corner of the structure. Return values TilePositions::Unknown if this unit is inaccessible. Returns ITilePosition object representing the unit's current tile position. See also getPosition, getInitialTilePosition
     */
    ...
    /**
     * Retrieves the unit's current Hit Points (HP) as seen in the game. Returns An integer representing the amount of hit points a unit currently has. Note In Starcraft, a unit usually dies when its HP reaches 0. It is possible however, to have abnormal HP values in the Use Map Settings game type and as the result of a hack over Battle.net. Such values include units that have 0 HP (can't be killed conventionally) or even negative HP (death in one hit). See also IUnitType::maxHitPoints, getShields, getInitialHitPoints
     */
    int getHitPoints();

    /**
     * Retrieves the unit's current Shield Points (Shields) as seen in the game. Returns An integer representing the amount of shield points a unit currently has. See also IUnitType::maxShields, getHitPoints
     */
    int getShields();

    /**
     * Retrieves the unit's current Energy Points (Energy) as seen in the game. Returns An integer representing the amount of energy points a unit currently has. Note Energy is required in order for units to use abilities. See also IUnitType::maxEnergy
     */
    int getEnergy();
    ...
    /**
     * Retrieves the frame number that sent the last successful command. Note This value is comparable to IGame::getFrameCount. Returns The frame number that sent the last successfully processed command to BWAPI. See also IGame::getFrameCount, getLastCommand
     */
    ...
    /**
     * Retrieves the IPlayer that last attacked this unit. Returns IPlayer interface object representing the player that last attacked this unit. Return values nullptr If this unit was not attacked. Note If this function returns a successful state, then the following function calls will also return a successful state: exists()
     */
    IPlayer getLastAttackingPlayer();
     ...
     */
    ITilePosition getInitialTilePosition();

    /**
     * Retrieves the amount of hit points that this unit started off with at the beginning of the game. The unit must be neutral. Returns Number of hit points that this unit started with. Return values 0 if this unit was not a neutral unit at the beginning of the game. Note : It is possible for the unit's initial hit points to differ from the maximum hit points. See also IGame::getStaticNeutralUnits
     */
    int getInitialHitPoints();

    /**
     * Retrieves the amount of resources contained in the unit at the beginning of the game. The unit must be a neutral resource container. Returns Amount of resources that this unit started with. Return values 0 if this unit was not a neutral unit at the beginning of the game, or if this unit does not contain resources. It is possible that the unit simply contains 0 resources. See also IGame::getStaticNeutralUnits
     */
    ...
    /**
     * Retrieves the building type that a worker (SCV, Probe, Drone) is about to construct. If the unit is morphing or is an incomplete structure, then this returns the IUnitType that it will become when it has completed morphing/constructing. Returns IUnitType indicating the type that a worker (SCV, Probe, Drone) is about to construct, or an incomplete unit will be when completed.
     */
    IUnitType getBuildType();

    /**
     * Retrieves the list of units queued up to be trained. Returns a IUnitType::set containing all the types that are in this factory's training queue. See also train, cancelTrain, isTraining
     */
    List<IUnitType> getTrainingQueue();

    /**
     * Retrieves the technology that this unit is currently researching. Returns ITechType indicating the technology being researched by this unit. Return values TechTypes::None if this unit is not researching anything. See also research, cancelResearch, isResearching, getRemainingResearchTime
     */
    ITechType getTech();

    /**
     * Retrieves the upgrade that this unit is currently upgrading. Returns IUpgradeType indicating the upgrade in progress by this unit. Return values UpgradeTypes::None if this unit is not upgrading anything. See also upgrade, cancelUpgrade, isUpgrading, getRemainingUpgradeTime
     */
    IUpgradeType getUpgrade();
    ...
    /**
     * Retrieves the amount of time until the unit is done researching its currently assigned ITechType. Returns The remaining research time, in frames, for the current technology being researched by this unit. Return values 0 If the unit is not researching anything. See also research, cancelResearch, isResearching, getTech
     */
     ...
     */
    IUnit getOrderTarget();

    /**
     * Retrieves the target position for the unit's order. For example, when Orders::Move is assigned, getTargetPosition returns the end of the unit's path, but this returns the location that the unit is trying to move to. Returns IPosition that this unit is currently targetting. See also getTargetPosition, getOrder
     */
    IPosition getOrderTargetPosition();

    /**
     * Retrieves the position the structure is rallying units to once they are completed. Returns IPosition that a completed unit coming from this structure will travel to. Return values Positions::None If this building does not produce units. Note If getRallyUnit is valid, then this value is ignored. See also setRallyPoint, getRallyUnit
     */
    IPosition getRallyPosition();

    /**
     * Retrieves the unit the structure is rallying units to once they are completed. Units will then follow the targetted unit. Returns IUnit that a completed unit coming from this structure will travel to. Return values nullptr If the structure is not rallied to a unit or it does not produce units. Note A rallied unit takes precedence over a rallied position. That is if the return value is valid(non-null), then getRallyPosition is ignored. See also setRallyPoint, getRallyPosition
     */
    IUnit getRallyUnit();

    /**
     * Retrieves the add-on that is attached to this unit. Returns IUnit interface that represents the add-on that is attached to this unit. Return values nullptr if this unit does not have an add-on.
     */
    IUnit getAddon();

    /**
     * Retrieves the Nydus Canal that is attached to this one. Every Nydus Canal can place a "Nydus Exit" which, when connected, can be travelled through by Zerg units. Returns IUnit interface representing the Nydus Canal connected to this one. Return values nullptr if the unit is not a Nydus Canal, is not owned, or has not placed a Nydus Exit.
     */
    IUnit getNydusExit();

    /**
     * Retrieves the power-up that the worker unit is holding. Power-ups are special units such as the Flag in the Capture The Flag game type, which can be picked up by worker units. Note If your bot is strictly melee/1v1, then this method is not necessary. Returns The IUnit interface object that represents the power-up. Return values nullptr If the unit is not carrying anything. Example BWAPI::Unitset myUnits = BWAPI::Broodwar->self()getUnits(); for ( auto u = myUnits.begin(); u != myUnits.end(); ++u ) { // If we are carrying a flag if ( u->getPowerUp() && u->getPowerUp()->getType() == BWAPI::UnitTypes::Powerup_Flag ) u->move( u->getClosestUnit(BWAPI::Filter::IsFlagBeacon && BWAPI::Filter::IsOwned) ); // return it to our flag beacon to score } Note If this function returns a successful state, then the following function calls will also return a successful state: getType().isWorker(), isCompleted()
     */
    IUnit getPowerUp();

    /**
     * Retrieves the Transport(Dropship, Shuttle, Overlord ) or Bunker unit that has this unit loaded inside of it. Returns IUnit interface object representing the Transport(Dropship, Shuttle, Overlord ) containing this unit. Return values nullptr if this unit is not in a Transport(Dropship, Shuttle, Overlord ).
     */
    IUnit getTransport();
    ...
    /**
     * This macro function checks if this unit is in the air. That is, the unit is either a flyer or a flying building. Returns true if this unit is in the air, and false if it is on the ground See also IUnitType::isFlyer, UnitInterface::isLifted
     */
    boolean isFlying();
    ...
    /**
     * Checks if this unit has been selected in the user interface. This function is only available if the flag Flag::UserInput is enabled. Returns true if this unit is currently selected, and false if this unit is not selected See also IGame::getSelectedUnits
     */
    boolean isSelected();
    ...
    /**
     * Performs some cheap checks to attempt to quickly detect whether the unit is unable to execute any commands (eg the unit is stasised). Return values true if BWAPI was unable to determine whether the unit can be commanded. false if an error occurred and the unit can not be commanded. See also IGame::getLastError, UnitInterface::canIssueCommand
     */
    boolean canCommand();

    /**
     * Performs some cheap checks to attempt to quickly detect whether the unit is unable to execute any commands as part of a Unitset (eg buildings, critters). Return values true if BWAPI was unable to determine whether the unit can be commanded grouped. false if an error occurred and the unit can not be commanded grouped. See also IGame::getLastError, UnitInterface::canIssueCommandGrouped, UnitInterface::canIssueCommand
     */
    boolean canCommandGrouped();

    /**
     * Cheap checks for whether the unit is able to execute an attack command to attack-move or attack a unit. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::attack, UnitInterface::canAttackMove, UnitInterface::canAttackUnit
     */
    boolean canAttack();

    /**
     * Checks whether the unit is able to execute an attack command to attack-move. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::attack
     */
    boolean canAttackMove();

    /**
     * Checks whether the unit is able to execute an attack command to attack-move, as part of a Unitset. See also IGame::getLastError, UnitInterface::canIssueCommandGrouped, UnitInterface::canAttackMove
     */
    boolean canAttackMoveGrouped();

    /**
     * Cheap checks for whether the unit is able to execute an attack command to attack a unit. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::attack
     */
    boolean canAttackUnit();

    /**
     * Cheap checks for whether the unit is able to execute a build command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::build
     */
    boolean canBuild();

    /**
     * Cheap checks for whether the unit is able to execute a buildAddon command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::buildAddon
     */
    boolean canBuildAddon();

    /**
     * Cheap checks for whether the unit is able to execute a train command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::train
     */
    boolean canTrain();

    /**
     * Cheap checks for whether the unit is able to execute a morph command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::morph
     */
    boolean canMorph();

    /**
     * Cheap checks for whether the unit is able to execute a research command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::research
     */
    boolean canResearch();

    /**
     * Cheap checks for whether the unit is able to execute an upgrade command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::upgrade
     */
    boolean canUpgrade();

    /**
     * Cheap checks for whether the unit is able to execute a setRallyPoint command to a position or unit. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::setRallyPoint, UnitInterface::canSetRallyPosition, UnitInterface::canSetRallyUnit.
     */
    boolean canSetRallyPoint();

    /**
     * Checks whether the unit is able to execute a setRallyPoint command to a position. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::setRallyPoint
     */
    boolean canSetRallyPosition();

    /**
     * Cheap checks for whether the unit is able to execute a setRallyPoint command to a unit. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::setRallyPoint
     */
    boolean canSetRallyUnit();

    /**
     * Checks whether the unit is able to execute a move command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::move
     */
    boolean canMove();

    /**
     * Checks whether the unit is able to execute a patrol command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::patrol
     */
    boolean canPatrol();

    /**
     * Cheap checks for whether the unit is able to execute a follow command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::follow
     */
    boolean canFollow();

    /**
     * Cheap checks for whether the unit is able to execute a gather command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::gather
     */
    boolean canGather();

    /**
     * Checks whether the unit is able to execute a returnCargo command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::returnCargo
     */
    boolean canReturnCargo();

    /**
     * Checks whether the unit is able to execute a holdPosition command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::holdPosition
     */
    boolean canHoldPosition();

    /**
     * Checks whether the unit is able to execute a stop command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::stop
     */
    boolean canStop();

    /**
     * Cheap checks for whether the unit is able to execute a repair command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::repair
     */
    boolean canRepair();

    /**
     * Checks whether the unit is able to execute a burrow command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::burrow
     */
    boolean canBurrow();
    ...
    /**
     * Cheap checks for whether the unit is able to execute a load command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::load
     */
    boolean canLoad();

    /**
     * Cheap checks for whether the unit is able to execute an unload command or unloadAll at current position command or unloadAll at a different position command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::unload, UnitInterface::unloadAll
     */
    boolean canUnloadWithOrWithoutTarget();

    /**
     * Cheap checks for whether the unit is able to execute an unload command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::unload
     */
    boolean canUnload();

    /**
     * Checks whether the unit is able to execute an unloadAll command for the current position. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::unloadAll
     */
    boolean canUnloadAll();

    /**
     * Cheap checks for whether the unit is able to execute an unloadAll command for a different position. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::unloadAll
     */
    boolean canUnloadAllPosition();

    /**
     * Checks whether the unit is able to execute a haltConstruction command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::haltConstruction
     */
    boolean canHaltConstruction();

    /**
     * Checks whether the unit is able to execute a cancelConstruction command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::cancelConstruction
     */
    boolean canCancelConstruction();

    /**
     * Checks whether the unit is able to execute a cancelAddon command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::cancelAddon
     */
    boolean canCancelAddon();

    /**
     * Checks whether the unit is able to execute a cancelTrain command for any slot. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::cancelTrain
     */
    boolean canCancelTrain();

    /**
     * Cheap checks for whether the unit is able to execute a cancelTrain command for an unspecified slot. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::cancelTrain
     */
    boolean canCancelTrainSlot();

    /**
     * Checks whether the unit is able to execute a cancelMorph command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::cancelMorph
     */
    boolean canCancelMorph();

    /**
     * Checks whether the unit is able to execute a cancelResearch command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::cancelResearch
     */
    boolean canCancelResearch();

    /**
     * Checks whether the unit is able to execute a cancelUpgrade command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::cancelUpgrade
     */
    boolean canCancelUpgrade();

    /**
     * Cheap checks for whether the unit is able to execute a useTech command without a target or or a useTech command with a target position or a useTech command with a target unit. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::useTech
     */
    boolean canUseTechWithOrWithoutTarget();

    /**
     * Cheap checks for whether the unit is able to execute a placeCOP command. See also IGame::getLastError, UnitInterface::canIssueCommand, UnitInterface::placeCOP
     */
    boolean canPlaceCOP();

    default boolean isDoingSomething() {
        return isGatheringGas() || isGatheringMinerals() ||
                isConstructing() || isMoving() || isAccelerating()
                || isMorphing() || isAttacking();
    }

    default boolean isFullyHealthy() {
     ...
     */
    default boolean canAttackThisKindOfUnit(IUnit otherUnit) {
     ...
     */
    default WeaponType getWeaponAgainst(IUnit target) {
     ...
     */
    default boolean isJustShooting() {
        return isAttackFrame() || isStartingAttack();
    }

    List<IUnit> getEnemyUnitsInWeaponRange();

    List<IUnit> getFriendlyUnitsInRadiusOfSight();

    List<IUnit> getResourceUnitsInRadiusOfSight();

    List<IUnit> getEnemyUnitsInRadiusOfSight();
